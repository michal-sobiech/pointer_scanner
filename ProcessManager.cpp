...
LPVOID ProcessManager::getProcessBaseAddress(HANDLE processHandle, DWORD processID){
...
std::vector<LPVOID> ProcessManager::firstCellFilter(HANDLE processHandle, std::vector<MEMORY_BASIC_INFORMATION> pagesToFilter, BYTE cmpValue){
    auto startTick = clock();
...
std::vector<LPVOID> ProcessManager::filterCells(HANDLE processHandle, std::vector<LPVOID> cellsToCheck, BYTE cmpValue){
    auto startTick = clock();
